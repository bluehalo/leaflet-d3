...
(function(global,factory){typeof exports==="object"&&typeof module!=="undefined"?factory(require("d3"),require("d3-hexbin"),require("leaflet")):typeof define==="function"&&define.amd?define(["d3","d3-hexbin","leaflet"],factory):factory(global.d3,global.d3.hexbin)})(this,function(d3,d3Hexbin){"use strict";
/**
...
/**
		 * Default options
		 */
options:{radius:12,opacity:.6,duration:200,colorScaleExtent:[1,undefined],radiusScaleExtent:[1,undefined],colorRange:["#f7fbff","#08306b"],radiusRange:[4,12],pointerEvents:"all"},
/**
...
var bounds=that._map.getBounds();var bins=that._hexLayout(data);
...
that._scale.color.domain(colorDomain);that._scale.radius.domain(radiusExtent);
/*
			 * Join
...
// Grid
enter.append("path").attr("class","hexbin-grid").attr("transform",function(d){return"translate("+d.x+","+d.y+")"}).attr("d",function(d){return that._hexLayout.hexagon(that.options.radius)}).attr("fill","none").attr("stroke","none").style("pointer-events",that.options.pointerEvents).on("mouseover",function(d,i){that._hoverHandler.mouseover.call(this,that,d,i);that._dispatch.call("mouseover",this,d,i)}).on("mouseout",function(d,i){that._dispatch.call("mouseout",this,d,i);that._hoverHandler.mouseout.call(this,that,d,i)}).on("click",function(d,i){that._dispatch.call("click",this,d,i)});
// Exit
...
var tooltip=d3.select("body").append("div").attr("class","hexbin-tooltip").style("z-index",9999).style("pointer-events","none").style("visibility","hidden").style("position","absolute");tooltip.append("div").attr("class","tooltip-content");
